...
async def _run_restyle(
    ...
    *,
    ...
    # The wrapper persists every completed page and reuses only pages whose
    # prompt hash still matches. A style change invalidates old prose, while a
    # repeated identical command can resume an interrupted run.
    ...
    async with get_session(sf) as session:
        ...
        # Include pages reused from a prior interrupted run. The persistence
        # wrapper saves them before the final FTS pass, but reused pages are not
        # returned in generated_pages.
